...
const { query, initDatabase } = require('./index');
...
async function seed() {
  ...
  if (parseInt(storeCount.rows[0].count, 10) > 0) {
    console.log('[Seed] Database already has stores. Seeding skipped.');
    return;
  }
  ...
  console.log('[Seed] Inserting Super Admin user...');
  await query(
  ...
  // 2. Create Store Managers & Staff
  console.log('[Seed] Inserting store managers & staff...');
  const starterMgr = await query(
    `INSERT INTO users (username, email, password_hash, full_name, phone_number, role)
     VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT (username) DO UPDATE SET password_hash = EXCLUDED.password_hash RETURNING id`,
  ...
  // -------------------------------------------------------------
  console.log('[Seed] Creating Store 1: Starter Plan (Mini Cafe)...');
  ...
  // -------------------------------------------------------------
  console.log('[Seed] Creating Store 2: Professional Plan (Somtum Zaab)...');
  ...
  // -------------------------------------------------------------
  console.log('[Seed] Creating Store 3: Enterprise Plan (Grand Buffet Shabu)...');
